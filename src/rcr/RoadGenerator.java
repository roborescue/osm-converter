...
public class RoadGenerator {
	...
	static String[] highwayValues = 
			new String[] { "primary", "primary_link", "secondary", "tertiary", "trunk", 
							"trunk_link", "motorway", "motorway_link", "residential", "unclassified", 
							"service", "road", "pedestrian", "footway", "living_street"  };
...
//				} 
	...
	private Way cutWay(Way w, Bounds b) {
		...
		if (w.getNode(0).getCoor().isWithin(b) && w.getNode(w.getNodesCount()-1).getCoor().isWithin(b)) {
		...
		for (Pair<Node, Node> p : w.getNodePairs(false)) {
			...
			if (p.a.getCoor().isWithin(b) != p.b.getCoor().isWithin(b)) {
				// Cut segment
				Line2D segment = new Line2D.Double(Vector.asPoint(p.a.getEastNorth()), Vector.asPoint(p.b.getEastNorth()));
				Rectangle2D rect = new Rectangle2D.Double(Math.min(b1.getX(), b2.getX()), Math.min(b1.getY(), b2.getY()), 
						Math.abs(b1.getX() - b2.getX()), Math.abs(b1.getY() - b2.getY()));
				...
				System.out.println("Cutting " + p.a.getEastNorth() + " -- " + p.b.getEastNorth());
				if (cut == null) {
					assert false;
//					nodes.add(outsideNode);
				}	
				else if (Vector.asPoint(outsideNode.getEastNorth()).distance(cut) <= MAX_CUT_OVERLAP) {
					// Segment may extend MAX_CUT_OVERLAP outside the bbox
					System.out.println("  extend outside" );
					nodes.add(outsideNode);
				}
				else if (Vector.asPoint(insideNode.getEastNorth()).distance(cut) <= MAX_CUT_OVERLAP) {
					...
					nodes.add(new Node(Main.getProjection().eastNorth2latlon(Vector.asEastNorth(cut))));
				}
			}
	...
	private boolean segmentInShape(Node n1, Node n2) {
		...
		Line2D l = new Line2D.Double(Vector.asPoint(n1.getEastNorth()), Vector.asPoint(n2.getEastNorth()));
		
		for (Way w : data.searchWays(bounds)) {
			if (RCRDataSet.isAreaShape(w) && AreaTools.lineContainedInPolygon(l, w)) {
	...
	public double getMaxRoadWidth(Way road, double increment) {
		double width = RCRDataSet.parseInt(road, "rcr:width", Constants.DEFAULT_LANE_WIDTH)/1000.;
